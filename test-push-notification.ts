...
import { NotificationType, NotificationChannel, NotificationPriority } from './src/app/modules/notification/notification.interface';
...
async function testPushNotification() {
  try {
    ...
    if (!user.deviceToken) {
      ...
      await User.findByIdAndUpdate(MOCK_USER_ID, { deviceToken: MOCK_DEVICE_TOKEN });
    }

    console.log(`Sending test notification to user ${user.name} (${user._id})...`);

    // 2. Create a notification with PUSH channel
    const notification = await NotificationServices.createNotification({
      userId: user._id,
      title: 'Photopia Test Push',
    ...
    console.log('Check your terminal logs for "Successfully sent message" or "Error sending message"');
