...
class MonitoringService {
    ...
    async monitorPaymentFailures() {
        try {
            const failedPayments = await subscription_model_1.Subscription.find({
    ...
    async monitorTrialConversions() {
        try {
            const trialEndingSoon = await subscription_model_1.Subscription.find({
                status: 'trialing',
                trialEnd: {
                    $gte: new Date(),
    ...
    async generateDailyReport() {
        try {
            ...
            const report = {
                ...
                subscriptions: {
                    total: await subscription_model_1.Subscription.countDocuments(),
                    active: await subscription_model_1.Subscription.countDocuments({ status: 'active' }),
                    trialing: await subscription_model_1.Subscription.countDocuments({ status: 'trialing' }),
                    pastDue: await subscription_model_1.Subscription.countDocuments({ status: 'past_due' }),
                    canceled: await subscription_model_1.Subscription.countDocuments({ status: 'canceled' }),
                },
                newSubscriptions: await subscription_model_1.Subscription.countDocuments({
                    createdAt: { $gte: yesterday, $lt: today },
                }),
                canceledSubscriptions: await subscription_model_1.Subscription.countDocuments({
                    status: 'canceled',
                    updatedAt: { $gte: yesterday, $lt: today },
                }),
                paymentFailures: await subscription_model_1.Subscription.countDocuments({
                    paymentFailureCount: { $gt: 0 },
                    updatedAt: { $gte: yesterday, $lt: today },
                }),
                plans: {
                    total: await subscription_plan_model_1.SubscriptionPlan.countDocuments(),
                    active: await subscription_plan_model_1.SubscriptionPlan.countDocuments({ isActive: true }),
                },
            };
    ...
    async monitorWebhookHealth() {
        try {
            ...
            const staleSubscriptions = await subscription_model_1.Subscription.find({
                status: { $in: ['active', 'trialing'] },
