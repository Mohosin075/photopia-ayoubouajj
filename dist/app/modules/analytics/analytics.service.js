...
const trackVisit = async (payload) => {
    await analytics_model_1.Analytics.create({
        ...
        serviceId: payload.serviceId ? new mongoose_1.Types.ObjectId(payload.serviceId) : undefined,
...
const getPremiumAnalytics = async (providerId) => {
    ...
    const totalVisits = await analytics_model_1.Analytics.distinct('visitorId', { providerId: objectId, type: 'view' });
    ...
    const userBookings = await booking_model_1.Booking.find({ providerId: objectId, status: 'completed' }).lean();
    ...
    const firstService = await service_model_1.Service.findOne({ providerId: objectId }).populate('category').lean();
