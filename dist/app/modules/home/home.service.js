...
const getHomeData = async (userId) => {
    ...
    if (mainCategories.length === 0) {
        mainCategories = await category_model_1.Category.find({ isActive: true, type: 'category' }).limit(3);
    }
    ...
    if (trendingThisWeek.length === 0) {
        trendingThisWeek = await category_model_1.Category.find({ isActive: true, type: 'subcategory' }).limit(3);
    }
    ...
    let creativeStyles = await category_model_1.Category.find({ theme: { $ne: null }, isActive: true })
    ...
    if (userLocation && userLocation.coordinates && userLocation.coordinates[0] !== 0) {
        ...
        for (const city of cities) {
            ...
            const service = await service_model_1.Service.findOne({ 'location.city': city._id, isActive: true })
                .select('coverMedia')
                .lean();
            nearYou.push({
                town: city._id,
    ...
    if (nearYou.length === 0) {
        ...
        const fallbackPipeline = [
            { $match: { roles: user_1.USER_ROLES.PROFESSIONAL, 'address.city': { $ne: null } } },
            { $group: {
                    _id: "$address.city",
                    count: { $sum: 1 }
                } },
    ...
    if (originalProjects.length === 0) {
        originalProjects = await service_model_1.Service.find({ status: service_1.SERVICE_STATUS.ACTIVE, isActive: true })
            .populate('providerId', 'name fullName profile isOnline')
            .populate('category', 'name image icon theme')
            .limit(3)
            .lean();
    ...
    // Section 10: Ideas
    let ideas = await inspiration_model_1.Inspiration.find().limit(6);
    if (ideas.length === 0) {
        ideas = await inspiration_model_1.Inspiration.find().limit(3);
    }
    return {
        recentlyViewed,
        mainCategories,
