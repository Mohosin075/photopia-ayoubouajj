...
const createOrUpdateAvailability = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user as JwtPayload
    if (!user) throw new ApiError(httpStatus.UNAUTHORIZED, 'User not found')
    const result = await AvailabilityService.createOrUpdateAvailability(
      user.userId,
      req.body,
    )
...
const checkDateAvailability = catchAsync(
  async (req: Request, res: Response) => {
    const { providerId } = req.params
    ...
    const result = await AvailabilityService.checkAvailabilityForDate(
      providerId,
      new Date(date),
      serviceId as string,
    )
...
const getTimeSlots = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!duration || typeof duration !== 'string') {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'Duration query parameter is required',
    )
  }
...
const getMonthCalendar = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await AvailabilityService.getMonthCalendar(
    providerId,
    monthNum,
    yearNum,
    serviceId as string,
