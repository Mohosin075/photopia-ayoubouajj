...
const calculatePrice = async (
  ...
  // Fallback to basic price if hourly rates are not set but pricing type is hourly
  if ((!baseRate || baseRate === 0) && service.pricingType === 'HOURLY') {
  ...
  } else if (service.pricingType === 'DAILY') {
  ...
  if (distanceFromProviderKm > (service.location?.serviceRadiusKm || 25)) {
    if (!service.allowOutsideRadius) {
       throw new ApiError(httpStatus.BAD_REQUEST, 'Location is outside service radius')
    }
