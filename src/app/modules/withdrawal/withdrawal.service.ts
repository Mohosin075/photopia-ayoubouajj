...
const createWithdrawal = async (
  ...
  const isStripeConnected =
    professionalProfile?.stripeAccountId &&
    professionalProfile?.stripeOnboardingComplete
  ...
  try {
    ...
    if (isStripeConnected) {
      ...
      try {
        ...
        throw new ApiError(
          httpStatus.BAD_REQUEST,
          `Stripe Payout failed: ${stripeError.message}`,
        )
      }
    } else {
      ...
      if (!payload.bankAccountDetails) {
        throw new ApiError(
          httpStatus.BAD_REQUEST,
          'Bank account details are required for manual withdrawal',
        )
      }
