...
const getProfile = async (userId: string) => {
  const profile = await ProfessionalProfile.findOne({ user: userId }).populate(
    'user',
  )
  if (!profile) {
  ...
  return {
    ...
    statistics: {
      ...
      revenue: {
        ...
        percentageChange: calculateChangeValue(
          currentMonthRevenue,
          lastMonthRevenue,
        ),
      },
...
const removeItem = async (
  userId: string,
  payload: {
    field: 'portfolio' | 'specialties' | 'language' | 'documents'
    values: string[]
  },
) => {
...
const getDetailedStatistics = async (userId: string) => {
  ...
  const viewsPerformance = calculateChange(
    profile.profileViews || 0,
    categoryAverageViews,
  )
  const categoryAverageRating = 4.2
  const ratingPerformance = calculateChange(
    profile.rating || 0,
  ...
  const [currentMonthData, previousMonthData, premiumMetrics] =
  ...
  const response: any = {
    ...
    revenueAnalytics: {
      ...
      bestPerforming:
        isPremium && formattedWeeklyRevenue.length > 0
...
const exportStatisticsReport = async (userId: string) => {
  ...
  stats.revenueAnalytics.weeklyBreakdown.forEach(
    (w: { week: string; amount: number }) => {
