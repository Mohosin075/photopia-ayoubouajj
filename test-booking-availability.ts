...
const runTest = async () => {
  ...
  try {
    ...
    const check1 = await AvailabilityService.checkAvailabilityForDate(providerId.toString(), d);
    console.log('Check Monday:', check1.isAvailable === true ? 'PASS' : 'FAIL');

    // Check Blocked Date (Tomorrow)
    const check2 = await AvailabilityService.checkAvailabilityForDate(providerId.toString(), tomorrow);
    console.log('Check Blocked Date:', check2.isAvailable === false ? 'PASS' : 'FAIL');

    // Check Price Override Date
    const check3 = await AvailabilityService.checkAvailabilityForDate(providerId.toString(), dayAfter);
    console.log('Check Override Date:', check3.isAvailable === true ? 'PASS' : 'FAIL');
    console.log('Check Override Price in Availability:', check3.pricing?.priceOverride === 200 ? 'PASS' : 'FAIL', check3.pricing);
