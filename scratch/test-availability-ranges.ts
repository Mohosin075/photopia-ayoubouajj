...
const runTest = async () => {
  ...
  try {
    ...
    console.log('Test 1 (Jan 17 - Blocked Range):', checkBlocked.isAvailable === false ? 'PASS' : 'FAIL', checkBlocked.reason);
    ...
    console.log('Test 3 (Jan 22 - Outside Periods):', checkOutside.isAvailable === false ? 'PASS' : 'FAIL', checkOutside.reason);
    ...
    console.log('Test 4.1 (Monday Fallback):', checkMonday.isAvailable === true ? 'PASS' : 'FAIL', checkMonday.workingHours);
    ...
    console.log('Test 4.2 (Tuesday Inactive):', checkTuesday.isAvailable === false ? 'PASS' : 'FAIL', checkTuesday.reason);

    // Clean up
    await Availability.deleteMany({ providerId: { $in: [providerId, providerId2] } });
